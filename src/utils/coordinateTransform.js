 ...
 * 1. 车辆坐标系（协议坐标系）：
 *    - 原点在左下角 (0, 0)
 *    - X轴：0 → 5.496m（向右为正，实物沙盘物理尺寸）
 *    - Y轴：0 → 4.0m（向上为正，对应模型Z轴变小）
 * 
...
/**
 * 沙盘模型逻辑尺寸（单位：米，与实物沙盘物理尺寸一致）
 * 车辆坐标系的范围：X: 0 → width, Y: 0 → depth
 */
export const LOGICAL_DIMENSIONS = {
    width: 5.496,   // X轴总宽度（米）- 实物沙盘物理尺寸
    depth: 4.0      // Z轴总深度（米）- 实物沙盘物理尺寸
};
...
export const PARKING_SLOTS = {
    1: { x: 3.206, y: 1.632, z: 0 },  // 1号车位
    2: { x: 3.532, y: 1.632, z: 0 }   // 2号车位
};
